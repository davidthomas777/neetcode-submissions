...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        // map each unqiue number to the count in the vector nums
        for (int num : nums) {
            myHash[num]++;
        }
        
        // Create a new vector of pairs (number, frequency)
        std::vector<std::pair<int, int>> freqPairs;
        for (const auto& pair : myHash) {
            freqPairs.push_back({pair.first, pair.second});
        }
        
        // Sort by frequency in descending order so you can stop when it hits k values
